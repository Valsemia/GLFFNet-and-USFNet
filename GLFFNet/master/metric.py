...
_EPS = np.spacing(1)#最大可表示的非零值
_TYPE = np.float64#用于指定预测值和真实值的类型

#_prepare_data() 函数用于准备预测值和真实值，使其适合用于后续的计算
...
class RMSE(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        rmse = self.cal_rmse(pred, gt)
        self.rmses.append(rmse)

    def cal_rmse(self, pred: np.ndarray, gt: np.ndarray) -> np.ndarray:

        rmse = np.sqrt(np.mean((pred - gt)**2))
        return rmse

    def get_results(self) -> dict:
        rmse = np.mean(np.array(self.rmses, _TYPE))
